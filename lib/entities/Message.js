...
/**
 * Get the moreAction of the notify
 * @return {string}
 */
Message.prototype.getMoreAction = function() {
  return this._getData('moreAction');
};

/**
 * Get the moreLabel of the notify
 * @return {string}
 */
Message.prototype.getMoreLabel = function() {
  return this._getData('moreLabel');
};
...
/**
 * Set the moreAction of the notify
 * @param {string} moreAction
 */
Message.prototype.setMoreAction = function(moreAction) {
  return this._setData('moreAction', moreAction);
};

/**
 * Set the moreLabel of the notify
 * @param {string} moreLabel
 */
Message.prototype.setMoreLabel = function(moreLabel) {
  return this._setData('moreLabel', moreLabel);
};
...
Message.prototype._loadDetails = function(details, callback) {
  ...
  this._setData('moreAction', details.moreAction);
  this._setData('moreLabel', details.moreLabel);
...
Message.prototype.hide = function() {
  ...
  return new Promise(function(resolve, reject) {
    that._getDriver().update({'hide': true}, function(err, result) {
      if (err) {
        return reject(new ErrorX(100, 'Error on hide message', err));
      }
      that._isLoad = false;
      resolve();
    });
...
Message.prototype.unhide = function() {
  var that = this;
  return new Promise(function(resolve, reject) {
    that._getDriver().update({'hide': false}, function(err, result) {
      if (err) {
        return reject(new ErrorX(100, 'Error on unhide message', err));
      }
      that._isLoad = false;
      resolve();
    });
...
Message.prototype.archive = function() {
  var that = this;
  return new Promise(function(resolve, reject) {
    that._getDriver().update({'archived': true}, function(err, result) {
      if (err) {
        return reject(new ErrorX(100, 'Error on archive message', err));
      }
      that._isLoad = false;
      resolve();
    });
...
Message.prototype.unarchive = function() {
  var that = this;
  return new Promise(function(resolve, reject) {
    that._getDriver().update({'archived': false}, function(err, result) {
      if (err) {
        return reject(new ErrorX(100, 'Error on unarchive message', err));
      }
      that._isLoad = false;
      resolve();
    });
...
Message.prototype.updateFirstView = function(date) {
  ...
  var that = this;
  return new Promise(function(resolve, reject) {
    that._getDriver().update({'firstView': date}, function(err, result) {
      if (err) {
        return reject(new ErrorX(100, 'Error on update first message', err));
      }
      that._isLoad = false;
      resolve();
    });
...
Message.prototype.updateConfirmView = function(date) {
  ...
  var that = this;
  return new Promise(function(resolve, reject) {
    that._getDriver().update({'confirmView': date}, function(err, result) {
      if (err) {
        return reject(new ErrorX(100, 'Error on confirmView message', err));
      }
      that._isLoad = false;
      resolve();
    });
